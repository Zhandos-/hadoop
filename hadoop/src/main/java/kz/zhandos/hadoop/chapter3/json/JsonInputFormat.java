package kz.zhandos.hadoop.chapter3.json;
...
import kz.zhandos.hadoop.util.HadoopCompat;
    ...
    public static boolean decodeLineToJson(JSONParser parser, Text line, MapWritable value) {
      ...
      try {
        ...
        while (jsonObj.keys().hasNext()) {
          Object key = jsonObj.keys().next();
          Text mapKey = new Text(key.toString());
          Text mapValue = new Text();
          if (jsonObj.get((String) key) != null) {
            mapValue.set(jsonObj.get((String) key).toString());
          }
