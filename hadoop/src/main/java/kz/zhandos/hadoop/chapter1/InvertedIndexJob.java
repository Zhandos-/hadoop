package kz.zhandos.hadoop.chapter1;
...
/**
 * A job which creates an inverted index. The index is a mapping from words to the files that
 * contain the word.
 */
public final class InvertedIndexJob extends Configured implements Tool {
  ...
  public int run(final String[] args) throws Exception {
    ...
    String url1 = InvertedIndexJob.class.getResource("1.txt").getFile();
    Path input = new Path(url1);
  ...
  /**
   * The reducer, which joins on all unique words in the input files, and emits all the files that
   * contained the word.
   */
